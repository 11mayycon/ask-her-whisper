...
router.post('/webhook', async (req, res) => {
  try {
    ...
    const instanceHeader = req.headers['x-evolution-instance'] as string;
...
async function handleMessagesUpsert(event: any, instanceId?: string) {
  ...
  for (const msg of messages) {
    try {
      ...
      try {
        const wsService = getWebSocketEventsService();
...
async function handleMessagesUpdate(event: any, instanceId?: string) {
  const db = await getDatabase();
  ...
  for (const update of updates) {
    try {
      ...
      await db.run(
        ...
        [status, finalInstanceId, messageId]
      );
...
async function handleChatsUpsert(event: any, instanceId?: string) {
  const db = await getDatabase();
  ...
  for (const chat of chats) {
    try {
      ...
      await db.run(
        `INSERT INTO whatsapp_chats (instance_id, chat_id, name, unreadCount, isGroup, lastMessageAt)
         VALUES (?, ?, ?, ?, ?, ?)
...
async function handleContactsUpsert(event: any, instanceId?: string) {
  const db = await getDatabase();
...
async function handleConnectionUpdate(event: any, instanceId?: string) {
  const db = await getDatabase();
