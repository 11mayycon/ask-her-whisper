...
router.get('/memory', async (req, res) => {
  try {
    ...
    const memory = await db.get(
      "SELECT * FROM ai_memory WHERE key = 'instructions' LIMIT 1"
    );
    res.json(memory);
  } catch (error) {
...
router.post('/memory', async (req, res) => {
  try {
    ...
    const existing = await db.get(
      "SELECT id FROM ai_memory WHERE key = 'instructions' LIMIT 1"
    );

    if (existing) {
      // Atualizar
      await db.run(
        "UPDATE ai_memory SET value = ?, updated_at = CURRENT_TIMESTAMP WHERE key = 'instructions'",
        [instructions]
      );
    } else {
      // Inserir
      const id = randomUUID();
      await db.run(
        "INSERT INTO ai_memory (id, key, value, category) VALUES (?, 'instructions', ?, 'system')",
        [id, instructions]
      );
    }
    ...
    try {
      const { salesBot } = await import('../services/sales-bot');
...
async function getSystemContext(): Promise<string> {
  try {
    ...
    const supportUsers = await db.all(
      'SELECT full_name, email, is_active, max_concurrent_chats FROM support_users'
    );

    // Get active attendances
    const activeAttendances = await db.all(
      `SELECT a.*, sr.customer_name, sr.customer_phone, u.full_name as support_name
       FROM attendances a
    ...
    // Get waiting rooms
    const waitingRooms = await db.all(
    ...
    const recentMessages = await db.all(
      `SELECT m.*, sr.customer_name, sr.customer_phone
       FROM messages m
    ...
    const topicAnalysis = await db.all(
    ...
    const whatsappStatus = await db.get(
      'SELECT status, phone_number, connected_at FROM whatsapp_connections ORDER BY updated_at DESC LIMIT 1'
    );
