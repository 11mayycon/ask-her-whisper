...
router.get('/:instanceName', authenticate, async (req, res) => {
    try {
        ...
        const chats = await whatsapp_1.whatsappService.getChats(instanceName);
        ...
        const connection = await db.get('SELECT * FROM whatsapp_connections WHERE instance_name = ?', [instanceName]);
        let attendances = [];
        if (connection) {
            attendances = await db.all('SELECT * FROM attendances WHERE whatsapp_connection_id = ? ORDER BY updated_at DESC', [connection.id]);
        }
        ...
        const conversations = chats.map((chat) => {
            ...
            const attendance = attendances.find((att) => att.client_phone === phone);
            return {
                id: remoteJid,
...
router.get('/:instanceName/messages/:chatId', authenticate, async (req, res) => {
    try {
        ...
        const evolutionMessages = await whatsapp_1.whatsappService.getMessages(instanceName, chatId);
...
router.post('/:instanceName/send', authenticate, async (req, res) => {
    try {
        const { instanceName } = req.params;
        ...
        const support = await db.get('SELECT * FROM supports WHERE id = ?', [req.user.id]);
        if (!support) {
        ...
        await whatsapp_1.whatsappService.sendMessage(phone, prefixedMessage);
        // Salvar no banco de dados
        const connection = await db.get('SELECT * FROM whatsapp_connections WHERE instance_name = ?', [instanceName]);
        if (connection) {
            // Buscar ou criar atendimento
            let attendance = await db.get('SELECT * FROM attendances WHERE client_phone = ? AND whatsapp_connection_id = ?', [chatId.replace('@s.whatsapp.net', ''), connection.id]);
            if (!attendance) {
                // Criar novo atendimento
                const attendanceId = crypto.randomUUID();
                await db.run(`INSERT INTO attendances (id, client_name, client_phone, status, assigned_to, admin_id, whatsapp_connection_id)
           VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                    attendanceId,
                    chatId.split('@')[0],
            ...
            // Salvar mensagem
            await db.run(`INSERT INTO messages (id, room_id, sender_id, sender_type, content, message_type, admin_id)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                crypto.randomUUID(),
                attendance.id,
                support.id,
...
router.post('/:instanceName/take/:chatId', authenticate, async (req, res) => {
    try {
        const { instanceName, chatId } = req.params;
        const db = await (0, sqlite_1.getDatabase)();
        const connection = await db.get('SELECT * FROM whatsapp_connections WHERE instance_name = ?', [instanceName]);
        ...
        await db.run(`UPDATE attendances
       SET assigned_to = ?, status = 'in_progress', updated_at = CURRENT_TIMESTAMP
       WHERE client_phone = ? AND whatsapp_connection_id = ?`, [support.id, chatId.replace('@s.whatsapp.net', ''), connection.id]);
        res.json({
            success: true,
...
router.post('/:instanceName/send-media', authenticate, upload.single('media'), async (req, res) => {
    try {
        const { instanceName } = req.params;
        ...
        await whatsapp_1.whatsappService.sendMediaMessage(phone, fileUrl, prefixedCaption);
        // Salvar no banco
        const connection = await db.get('SELECT * FROM whatsapp_connections WHERE instance_name = ?', [instanceName]);
        if (connection) {
            let attendance = await db.get('SELECT * FROM attendances WHERE client_phone = ? AND whatsapp_connection_id = ?', [chatId.replace('@s.whatsapp.net', ''), connection.id]);
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            ...
            // Salvar mensagem
            await db.run(`INSERT INTO messages (id, room_id, sender_id, sender_type, content, message_type, media_url, admin_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                crypto.randomUUID(),
                attendance.id,
                support.id,
