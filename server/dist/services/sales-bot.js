...
exports.salesBot = exports.SalesBotService = void 0;
...
- Seja flexível e contextual`;
    ...
    async loadLeads() {
        try {
            const db = await (0, sqlite_1.getDatabase)();
            const savedLeads = await db.all('SELECT * FROM sales_leads WHERE last_message_at > datetime("now", "-7 days")');
    ...
    // Carregar memória do chat
    async loadChatMemory(phone) {
        try {
            const db = await (0, sqlite_1.getDatabase)();
            const memory = await db.get('SELECT * FROM chat_memory WHERE phone = ? LIMIT 1', [phone]);
            if (memory) {
                return {
    ...
    async saveChatMemory(lead) {
        try {
            const db = await (0, sqlite_1.getDatabase)();
            await db.run(`INSERT OR REPLACE INTO chat_memory
    ...
    async saveLead(lead) {
        try {
            const db = await (0, sqlite_1.getDatabase)();
            await db.run(`INSERT OR REPLACE INTO sales_leads
...
- Nunca mencione aspectos técnicos do sistema`;
    ...
    async updateChatMemory(lead, userMessage, aiResponse) {
        try {
...
exports.SalesBotService = SalesBotService;
// Exportar instância singleton
exports.salesBot = new SalesBotService();
