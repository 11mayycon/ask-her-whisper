...
exports.evolutionProxy = exports.EvolutionProxyService = void 0;
const axios_1 = __importDefault(require("axios"));
const sqlite_1 = require("../config/sqlite");
class EvolutionProxyService {
    constructor() {
        this.instances = new Map();
    }
    // Obter instância ativa do admin
    async getAdminInstance(adminId) {
        const db = await (0, sqlite_1.getDatabase)();
        const instance = await db.get(`SELECT * FROM whatsapp_instances
       WHERE admin_user_id = ?
    ...
    getClient(baseUrl, apiKey) {
        ...
        if (!this.instances.has(key)) {
            this.instances.set(key, axios_1.default.create({
                baseURL: baseUrl,
                headers: {
                    'apikey': apiKey,
    ...
    mapMessage(evo, chatId, instanceId) {
    ...
    // ========== ENDPOINTS PROXY ==========
    // Listar contatos
    async fetchContacts(instanceId, baseUrl, apiKey, cursor) {
        ...
        try {
           ...
           ON CONFLICT(instance_id, wa_id) DO UPDATE SET
             ...
             updated_at = CURRENT_TIMESTAMP`, [mapped.instance_id, mapped.wa_id, mapped.name, mapped.pushname, mapped.isBusiness, mapped.profilePicUrl]);
            }
            return {
    ...
    // Listar chats
    async fetchChats(instanceId, baseUrl, apiKey, limit = 50, cursor) {
        const client = this.getClient(baseUrl, apiKey);
        const db = await (0, sqlite_1.getDatabase)();
        try {
            ...
            for (const chat of chats) {
                ...
                await db.run(`INSERT INTO whatsapp_chats (instance_id, chat_id, name, unreadCount, isGroup, lastMessageAt, lastMessageText, lastMessageFrom)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)
           ON CONFLICT(instance_id, chat_id) DO UPDATE SET
             ...
             updated_at = CURRENT_TIMESTAMP`, [mapped.instance_id, mapped.chat_id, mapped.name, mapped.unreadCount,
                    mapped.isGroup, mapped.lastMessageAt, mapped.lastMessageText, mapped.lastMessageFrom]);
            }
            // Retornar do banco ordenado
            const savedChats = await db.all(`SELECT * FROM whatsapp_chats
         WHERE instance_id = ?
    ...
    async fetchMessages(instanceId, chatId, baseUrl, apiKey, limit = 50, cursor) {
        const client = this.getClient(baseUrl, apiKey);
        const db = await (0, sqlite_1.getDatabase)();
        try {
            ...
            for (const message of messages) {
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
             ON CONFLICT(instance_id, message_id) DO NOTHING`, [mapped.instance_id, mapped.chat_id, mapped.message_id, mapped.from_me,
            ...
            // Retornar do banco ordenado
            const savedMessages = await db.all(`SELECT * FROM whatsapp_messages
         WHERE instance_id = ? AND chat_id = ?
         ORDER BY timestamp ASC
         LIMIT ?`, [instanceId, chatId, limit]);
            return {
                data: savedMessages,
                nextCursor: null
            };
    ...
    // Enviar mensagem
    async sendMessage(instanceId, chatId, text, supportName, baseUrl, apiKey) {
        const client = this.getClient(baseUrl, apiKey);
        try {
            // Prefixar mensagem
    ...
    async getInstanceStatus(instanceId, baseUrl, apiKey) {
        const client = this.getClient(baseUrl, apiKey);
        try {
         ...
         WHERE instance_id = ?`, [state === 'open' ? 'connected' : state === 'connecting' ? 'connecting' : 'disconnected', instanceId]);
            return {
                status: state,
                state: response.data
            };
...
exports.EvolutionProxyService = EvolutionProxyService;
exports.evolutionProxy = new EvolutionProxyService();
